...
//lkd
	if(num_regulator == 6) {
		i2c_reg_write(RK818_I2C_ADDR, rk818_LDO_SET_VOL_REG(6), 0x3);
		printf("++++ LDO3 write 0x03 to regular \n");
		return 0;
	}
	if (num_regulator == 12) {
...
static int rk818_parse_dt(const void* blob)
{
	...
	reg_val = *(((u32 *)RKIO_GRF_PHYS) + 0x1cc/4);
	if((3 == ((reg_val >> 13) & 0x7)))
	{
		grf_writel((1<<16) | (0<<0) , GRF_GPIO2A_IOMUX);
		gpio_direction_output(GPIO_BANK2|GPIO_A0, 1);
		printf("v01: power_hold GPIO pull up \n");
	}

	gpio_direction_output(GPIO_BANK0|GPIO_D1, 1);
	printf("cpu_det GPIO pull up \n");
