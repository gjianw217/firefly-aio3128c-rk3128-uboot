...
static int rk816_parse_dt(const void *blob)
{
    ...
    struct fdt_gpio_state det_gpio;
	u32 bus, addr;
	int ret, i;
	...
	fdtdec_decode_gpios(blob, node, "cpu_det_gpio", &det_gpio, 1);
    cpu_det_gpio = det_gpio.gpio;
    if (!gpio_is_valid(cpu_det_gpio)) {
        ...
        if (ret < 0) {
            printf("Failed req gpio %d with ret=%d\n",
                    cpu_det_gpio, ret);
            return 0;
        }
        gpio_direction_output(cpu_det_gpio, 1);
        printf("cpu_det_gpio up\n");
    }
